...
contract CamuToken is ERC20, Ownable {
    ...
    constructor(
        ...
        _mint(_founderWallet, initialSupply);
    }
...
// File: contracts/CammunityDAO.sol
    ...
    function executeProposal(uint256 proposalId) external {
        ...
        if (proposal.proposalType == ProposalType.FUNDING) {
            ...
            if (proposal.amount + monthlySpent > maxMonthlySpend) {
                require(
                    quorum >= 67,
                    "Special quorum required for excess budget"
                );
            }
        ...
        if (proposal.proposalType == ProposalType.TERMINATION) {
            ...
            require(
                proposal.tokenVotes >= (totalVotes * 75) / 100,
                "Termination approval too low"
            );
            require(
                block.timestamp >= proposal.voteStart + 7 days,
                "Delay not met"
            );
        }
